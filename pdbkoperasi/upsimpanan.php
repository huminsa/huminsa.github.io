...
<style>
	...
	<label>Ubah Data Simpanan : <?php echo $_GET['id_simpan']; ?></label>
	<?php 
	include 'connect.php';
	if(isset($_POST['simpan'])){  // untuk proses simpan
		include 'connect.php';
		$sql =  "update id_simpan set id_simpan='".$_POST['id_simpan']."', tgl_simpan='".$_POST['tgl_simpan']."', id_anggota='".$_POST['id_anggota']."',id_karyawan='".$_POST['id_karyawan']."', ";
		$sql .= "jumlah_simpan='".$_POST['jumlah_simpan']."', bunga_simpan='".$_POST['bunga_simpan']."' where id_simpan='".$_GET['id_simpan']."' ";	
		$prose = mysqli_query($con, $sql);
		echo "<script>alert('data berhasil diubah');window.location='idxsimpanan.php';</script>";
	}
	// untuk pembacaan data dari tabel sesuai id-simpan
	$sql = "SELECT * from simpanan where id_simpan='".$_GET['id_simpan']."'";
